...
//callback function
type SQSOnMessageCallback func(msg *sqs.Message) error

const (
	// SyncConsumer to consume messages one by one.
	...
	// AsyncConsumer to consume messages at the same time.
...
type Consumer struct {
	...
	onMessage SQSOnMessageCallback
}

func NewConsumer(client MessageClient, config ConsumerConfig, onMessage SQSOnMessageCallback) Consumer {
	return Consumer{
		client:    client,
		config:    config,
		onMessage: onMessage,
